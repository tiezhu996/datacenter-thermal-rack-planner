...
func (s *Service) Login(ctx context.Context, req LoginRequest) (LoginResponse, error) {
	...
	if err := bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(req.Password)); err != nil {
		return LoginResponse{}, web.Internal(fmt.Errorf("password mismatch: %w", err))
	}
...
func (s *Service) Parse(raw string) (Claims, error) {
	...
	if err != nil || !parsed.Valid {
		return Claims{}, web.Forbidden("session token is invalid or expired")
	}
...
func (s *Service) CurrentPrincipal(ctx context.Context, id uint) (User, error) {
	...
	if err != nil {
		return User{}, web.Internal(fmt.Errorf("session account lookup: %w", err))
	}
	if !user.Active {
		return User{}, web.Internal(fmt.Errorf("session account is inactive"))
	}
	return user, nil
}
