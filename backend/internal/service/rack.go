...
func (s *RackService) Update(ctx context.Context, id uint, req dto.UpdateRackRequest, actor audit.Entry) (dto.RackResponse, error) {
	...
	updated := model.Rack{
		ID: id, ZoneID: req.ZoneID, RackCode: "", RowIndex: req.RowIndex,
	...
	actor.EntityType = "rack"
	actor.BeforeSummary = fmt.Sprintf("zone=%d position=%d,%d version=%d", req.ZoneID, req.RowIndex, req.ColumnIndex, req.Version)
	...
	return dto.RackResponse{
		ID: id, ZoneID: req.ZoneID, RackCode: updated.RackCode, RowIndex: req.RowIndex,
		ColumnIndex: req.ColumnIndex, PowerLimitKW: req.PowerLimitKW, AirflowLimitCFM: req.AirflowLimitCFM,
		RackUnits: req.RackUnits, RackStatus: req.RackStatus, Version: req.Version + 1,
		Utilization: dto.RackUtilization{},
	}, nil
}
